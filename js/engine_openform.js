...
function SaveDocument(WF_Action, msg, WF_Params) {
    ...
    if (isValid) {
        ...
        var r = formonsubmit();
        if (typeof(r) == "boolean" && r == false) { return; } else if (typeof(r) == "string") { formData += "&" + r; } //如果返回的时字符串则作为提交的附加数据
...
function GoToNextNode() {
    ...
    $('input[name="WF_NextNodeSelect"]:checked').each(function() {
        if (NodeList == "") { NodeList = $(this).val(); } else { NodeList += "," + $(this).val(); }
    });
...
function MobileDeleteNodeUser(nodeid, userid) {
    ...
    for (var i = 0; i < userArray.length; i++) {
        if (userArray[i] != userid) {
            if (newUserList == "") { newUserList = userArray[i]; } else { newUserList += "," + userArray[i]; }
        }
    }
